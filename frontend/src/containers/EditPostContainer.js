...
class EditPostContainer extends Component {
    ...
    render(){
        const {allPosts, allCategories, idPost, errorPost, postSuccess, loadingPost, category} = this.props
        ...
        return(
            <div>
                ...
                {(allPosts && idPost) &&
                    allPosts.map(post => {
                        if(post.id === idPost){
                            return(<FormEditPost key={idPost} post={post} allCategories={allCategories} onSubmit={this.handleSubmitPost} />)
                        }
                    })}
                    {errorPost && (<div>{errorPost}</div>)}
                    {postSuccess && (<Redirect to={`/${category}/${idPost}`}/>)}
